...
SELECT o.OrderID, e.EmployeeID, CONCAT(e.FirstName, ' ', e.LastName) AS Full_Name
FROM orders o
JOIN employees e
ON o.EmployeeID = e.EmployeeID
...
JOIN (
    ...
    FROM products
) min_price ON p.Price = min_price.MinPrice;
...
SELECT CustomerName, SUBSTRING_INDEX(CustomerName," ",1) AS First_Name, SUBSTRING_INDEX(CustomerName," ",-1) AS Last_Name
FROM customers
